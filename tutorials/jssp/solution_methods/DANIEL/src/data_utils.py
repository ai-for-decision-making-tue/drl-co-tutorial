...
def daniel_jssp_instance_to_job_shop(
        instance,
        instance_name="generated_daniel_jssp_instance"):
    """Convert a standard DANIEL JSSP instance into a benchmark ``JobShop``.

    DANIEL represents an instance as ``(job_length, op_pt)``. ``job_length``
    contains the number of operations in every job. ``op_pt`` has shape
    ``(number_of_operations, number_of_machines)`` and contains a positive
    processing time only for machines that can process an operation. For a
    standard JSSP, every operation has exactly one positive entry and each job
    visits every machine exactly once.
    """
    try:
        number_of_arrays = len(instance)
    except TypeError as error:
        raise ValueError("instance must be a (job_length, op_pt) pair") from error
    if number_of_arrays != 2:
        raise ValueError("instance must be a (job_length, op_pt) pair")

    job_length = np.asarray(instance[0])
    op_pt = np.asarray(instance[1])
    if job_length.ndim != 1:
        raise ValueError("job_length must be a 1-D array")
    if op_pt.ndim != 2:
        raise ValueError("op_pt must be a 2-D array")
    if job_length.size == 0 or 0 in op_pt.shape:
        raise ValueError("an instance must contain jobs, operations, and machines")
    if not np.issubdtype(job_length.dtype, np.integer):
        raise ValueError("job lengths must be integers")
    if not np.issubdtype(op_pt.dtype, np.number):
        raise ValueError("processing times must be numeric")
    if not np.all(np.isfinite(op_pt)) or np.any(op_pt < 0):
        raise ValueError("processing times must be finite and nonnegative")

    n_j = job_length.size
    n_op, n_m = op_pt.shape
    if np.any(job_length != n_m):
        raise ValueError(
            "a standard JSSP instance must have exactly n_m operations per job"
        )
    if int(job_length.sum()) != n_op:
        raise ValueError("the sum of job_length must equal the number of op_pt rows")
    if np.any(np.count_nonzero(op_pt, axis=1) != 1):
        raise ValueError(
            "every standard JSSP operation must have exactly one assigned machine"
        )

    assigned_machines = np.argmax(op_pt, axis=1)
    first_operation = 0
    for length in job_length:
        last_operation = first_operation + int(length)
        if not np.array_equal(
                np.sort(assigned_machines[first_operation:last_operation]),
                np.arange(n_m)):
            raise ValueError("every job must visit each machine exactly once")
    ...
    operation_id = 0
    for job_id, number_of_operations in enumerate(job_length):
        job = Job(job_id)
        previous_operation = None

        for _ in range(int(number_of_operations)):
            operation = Operation(job, job_id, operation_id)
            machine_id = int(assigned_machines[operation_id])
            duration = op_pt[operation_id, machine_id].item()
            operation.add_operation_option(machine_id, duration)
            operation.add_predecessors(
                [] if previous_operation is None else [previous_operation]
            )
    ...
    job_shop.add_sequence_dependent_setup_times(
        [[[0] * n_op for _ in range(n_op)] for _ in range(n_m)]
    )
...
def uniform_jssp_instance_generator(
        n_j,
        n_m,
        low,
        high,
        *,
        ...
        instance_name=None):
    """Generate a standard JSSP instance for DANIEL training and plotting.

    The default return value, ``(job_length, op_pt)``, can be passed directly to
    ``FJSPEnvForSameOpNums.set_initial_data`` after adding a batch dimension.
    Set ``return_job_shop=True`` to also receive an equivalent benchmark
    environment. Unlike the L2D generator, ``high`` is inclusive to match
    DANIEL's existing SD2 generator and configuration.
    """
    ...
    number_of_operations = n_j * n_m
    job_length = np.full(n_j, n_m, dtype=np.int32)
    processing_times = random_state.randint(
        low=low,
        high=high + 1,
        size=number_of_operations,
    )
    machine_order = np.vstack([
        random_state.permutation(n_m) for _ in range(n_j)
    ]).reshape(-1)
    op_pt = np.zeros((number_of_operations, n_m), dtype=np.int32)
    op_pt[np.arange(number_of_operations), machine_order] = processing_times
    daniel_instance = (job_length, op_pt)

    if not return_job_shop:
        return daniel_instance

    name = instance_name or f"uniform_jssp_{n_j}x{n_m}_seed_{seed}"
    job_shop = daniel_jssp_instance_to_job_shop(
        daniel_instance,
        instance_name=name,
    )
    return daniel_instance, job_shop
