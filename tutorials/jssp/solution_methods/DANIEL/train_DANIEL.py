...
from solution_methods.DANIEL.src.data_utils import (
    CaseGenerator,
    SD2_instance_generator,
    load_data_from_files,
    uniform_jssp_instance_generator,
)
...
class Trainer:
    def __init__(self, config, device):
        ...
        method_root = base_path / "solution_methods" / "DANIEL"
        self.save_dir = method_root / "save" / self.data_source
        self.train_log_dir = method_root / "train_log" / self.data_source
        ...
        if self.data_source == "SD1":
            ...
            self.data_name = f"{self.n_j}x{self.n_m}+jssp"
        else:
            raise ValueError(
                f"Unsupported DANIEL data source: {self.data_source}. "
                "Expected SD1, SD2, or JSSP."
            )

        training_data_root = (
            base_path / "solution_methods" / "DANIEL" / "training_data"
        )
        self.vali_data_path = (
            training_data_root / "data_train_vali" / self.data_source / self.data_name
        )
        self.test_data_path = training_data_root / self.data_source / self.data_name
        self.validation_description = str(self.vali_data_path)
        ...
        if self.data_source == "JSSP":
            self.test_data = ([], [])
            validation_config = config.get("JSSP_data_generation", {})
            validation_size = validation_config.get("validation_size", 100)
            if not isinstance(validation_size, int) or validation_size <= 0:
                raise ValueError("JSSP validation_size must be a positive integer")
            validation_seed = config["seed"]["seed_train_vali_datagen"]
            self.validation_description = (
                f"{validation_size} in-memory standard JSSP instances "
                f"(seed {validation_seed})"
            )
            vali_data = ([], [])
            for instance_index in range(validation_size):
                job_length, op_pt = uniform_jssp_instance_generator(
                    n_j=self.n_j,
                    n_m=self.n_m,
                    low=self.low,
                    high=self.high,
                    seed=validation_seed + instance_index,
                )
                vali_data[0].append(job_length)
                vali_data[1].append(op_pt)
        else:
            self.test_data = load_data_from_files(self.test_data_path)
            vali_data = load_data_from_files(self.vali_data_path)
            if not vali_data[0]:
                raise FileNotFoundError(
                    f"No DANIEL validation instances found at {self.vali_data_path}"
                )
    ...
    def train(self):
        ...
        print(f"vali data :{self.validation_description}")
        print("\n")
    ...
    def save_training_log(self):
        ...
        file_writing_obj = open(
            self.train_log_dir / f"reward_{self.model_name}.txt",
            "w",
        )
        file_writing_obj.write(str(self.log))

        file_writing_obj1 = open(
            self.train_log_dir / f"valiquality_{self.model_name}.txt",
            "w",
        )
    ...
    def sample_training_instances(self):
        ...
        for i in range(self.num_envs):
            if self.data_source == "SD1":
                ...
                JobLength, OpPT = uniform_jssp_instance_generator(
                    n_j=self.n_j,
                    n_m=self.n_m,
                    low=self.low,
                    high=self.high,
    ...
    def save_model(self):
        ...
        torch.save(
            ...
            self.save_dir / f"{self.model_name}.pth",
        )
    ...
    def load_model(self):
        ...
        """
        model_path = self.save_dir / f"{self.model_name}.pth"
