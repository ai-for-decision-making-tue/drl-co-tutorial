...
from solution_methods.L2D.src.normalization import (
    build_operation_features,
    get_reward_normalization_factor,
)
...
default_env_parameters = env_parameters


class NipsJSPEnv_test():
    def __init__(self, n_j: int, n_m: int, env_parameters=None):

        self.env_parameters = (
            default_env_parameters if env_parameters is None else env_parameters
        )
        self.normalize_reward_and_state = self.env_parameters.get(
            "normalize_reward_and_state", False
        )
        self.step_count = 0
    ...
    def _build_features(self):
        return build_operation_features(
            self.JSM_LBs,
            self.JSM_finished_mark,
            self.normalize_reward_and_state,
            self.env_parameters["et_normalize_coef"],
        )
    ...
    def reset(self, JSM_env: JobShop):
        ...
        self.JSM_max_endTime = (
            self.JSM_LBs.max()
            if not self.env_parameters["init_quality_flag"]
            else 0
        )
        ...
        processing_times = [
            processing_time
            for operation in self.JobShopModule.operations
            for processing_time in operation.processing_times.values()
        ]
        self.reward_normalization_factor = get_reward_normalization_factor(
            processing_times, self.normalize_reward_and_state
        )
    ...
    def step(self, action):
        ...
        # prepare for return
        feature_JSM = self._build_features()
        reward_JSM = self.normalize_reward(
            -(self.JSM_LBs.max() - self.JSM_max_endTime)
        )

        if reward_JSM == 0:
            reward_JSM = self.normalize_reward(self.env_parameters["rewardscale"])
