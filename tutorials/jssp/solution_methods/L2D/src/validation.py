...
def validate(vali_set, model, env_parameters_override=None,
             model_parameters_override=None, device_override=None):
    active_env_parameters = env_parameters_override or env_parameters
    active_model_parameters = model_parameters_override or model_parameters
    ...
    env = SJSSP(
        n_j=N_JOBS,
        n_m=N_MACHINES,
        env_parameters=active_env_parameters,
    )
    device = device_override or torch.device(active_env_parameters["device"])
    g_pool_step = g_pool_cal(graph_pool_type=active_model_parameters["graph_pool_type"],
    ...
    for data in vali_set:
        ...
        rewards = env.normalize_reward(-env.initQuality)
        while True:
            ...
            if done:
                break
        make_spans.append(env.denormalize_reward(rewards - env.posRewards))
