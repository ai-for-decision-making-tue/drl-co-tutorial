...
class SJSSP(gym.Env, EzPickle):
    ...
    def step(self, action):
        ...
        reward = self.normalize_reward(-(self.LBs.max() - self.max_endTime))
        if reward == 0:
            reward = self.normalize_reward(self.env_parameters["rewardscale"])
    ...
    def reset(self, data):
        ...
        self.reward_normalization_factor = get_reward_normalization_factor(
            self.dur, self.normalize_reward_and_state
        )
        ...
        self.initQuality = (
            self.LBs.max() if not self.env_parameters["init_quality_flag"] else 0
        )
        ...
        self.mchsStartTimes = -self.env_parameters["high"] * np.ones_like(
            self.dur.transpose(), dtype=np.int32
        )
        # Ops ID on machines
