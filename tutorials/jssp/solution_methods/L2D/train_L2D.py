...
def train_L2D(**parameters):
    ...
    envs = [
        SJSSP(n_j=n_job, n_m=n_machine, env_parameters=env_parameters)
        for _ in range(train_parameters["num_envs"])
    ]

    # Load validation data
    data_generator = uniform_instance_generator
    file_path = base_path / 'solution_methods' / 'L2D' / 'training_data'
    validation_path = file_path / (
        f"generatedData{n_job}_{n_machine}_Seed{env_parameters['np_seed_validation']}.npy"
    )
    ...
    for i_update in range(train_parameters["max_updates"]):
        ...
        for i, env in enumerate(envs):
            ...
            ep_rewards[i] = env.normalize_reward(-env.initQuality)
        ...
        if (i_update + 1) % 100 == 0:
            validation_result = -validate(
                vali_data,
                ppo.policy,
                env_parameters_override=env_parameters,
                model_parameters_override=model_parameters,
